				...
				<div class="col-lg-9 col-12 order-1 order-lg-2">
					...
					<div class="row">
						...
						<?php
								include("connection.php");
								//error_reporting(0);
                                $query = "Select * from tblshopregistration where 1=1";
								if(isset($_GET['cat']))
								{
									$query .= " and ShopCategory=" . $_GET['cat'];
									$catid=$_GET['cat'];
								}
								
								if($srchFLD<>"")
								{
									$query .= " and (ShopName like '%" . $srchFLD . "%' or ShopDescription like '%" . $srchFLD . "%' ) ";
								}
								...
								<ul class="prize d-flex">
									...
									<?php
												...
												if(isset($catid))
													{
                                  			  			$sql = "Select * from tblcategorytable" ;
                                 			  			$sql .= " Where SysId in (select ShopCategory from tblshopregistration where ShopCategory=$catid) ";
                               				 			 //   $sql .= " and cat_id<>" . $Id;
													}
													else
													{
														$sql = "Select * from tblcategorytable" ;
														$sql .= " Where SysId in (select ShopCategory from tblshopregistration where SysId=$id1) ";
													}
